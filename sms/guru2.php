...
$fhtml="0";
$te="0";
$report= '<div class="content">User: <span class="number"><b>'.$uid.'</b></span><br>';
flush();
...
if(!$id)
{
$report.=('Login Failed. We Didnot Get Session Value.');
goto end;
}
//$report.=('Login Successfull.');
goto bal;
}
elseif(stristr($lhtml,'Location: http://site2.way2sms.com/entry'))
{

$report.=('Login Failed. Invalid Login Details');
goto end;
}
else
{
$report.=('Login Failed. Unknown Error Occured');
goto end;
}
bal:
///$ch = curl_init();
...
{ $report.=('<br />SmS Sent Successfully To '.$to);
}
else
{$report.=('Error in Sending SmS.');
}
curl_close($ch);
